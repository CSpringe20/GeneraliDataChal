...
def recommend_coverages(new_customer_df, nn_neighbors=5, num_cov_ret=5):
    ...
    """
    new_customer_df['cod_sesso'] = new_customer_df['cod_sesso'].astype('category').cat.codes
    new_customer_df['cod_provincia_residenza'] = new_customer_df['cod_provincia_residenza'].astype('category').cat.codes

    # select all the "flag_" and convert yes/no to 1/0
    flag_cols = [col for col in new_customer_df.columns if col.startswith("flag_")]
    for col in flag_cols:
        new_customer_df[col] = new_customer_df[col].str.strip().str.lower().map({'si': 1, 'no': 0})
